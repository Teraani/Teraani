// This component is currently not in use due to causing layout issues.
// The navigation logic has been simplified and moved to the avatar dropdown
// in DashboardView.tsx. This file is kept for potential future rework.
"use client"
  ...
  (
    ...
    return (
      <div
        ref={ref}
        data-state={state}
        className={cn("hidden md:flex flex-col h-screen bg-background border-r text-foreground transition-all duration-300", 
...
>(({ className, asChild = false, ...props }, ref) => {
  ...
  return (
    <Comp
      ...
      className={cn(
        "duration-200 flex h-8 shrink-0 items-center rounded-md px-2 text-xs font-medium text-muted-foreground outline-none ring-ring transition-opacity ease-linear focus-visible:ring-2",
      ...
      <>
        {React.Children.map(children, child =>
          React.isValidElement(child) && child.type === 'svg'
            ? React.cloneElement(child as React.ReactElement, { className: 'size-4 shrink-0' })
            : null
        )}
        <span className={cn('truncate', state === 'collapsed' && 'invisible')}>
            {React.Children.map(children, child =>
